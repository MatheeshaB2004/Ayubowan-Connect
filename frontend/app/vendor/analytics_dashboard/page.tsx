...
export default async function Page({
  ...
  const ratingTrendRes = await fetch(
    `http://localhost:3001/dashboard/vendor/ratings-trend?userId=${userId}&period=${period}`,
    { cache: "no-store" }
  );

  const ratingTrend = await ratingTrendRes.json();
