  ...
  return (
    ...
    >
      <div className="relative h-36 overflow-hidden bg-[#e8f5f2]">
        {event.imageUrl ? (
          <Image src={event.imageUrl} alt={event.title} fill className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <div className="p-3">
        <h4 className="font-semibold text-gray-900 text-[13px] leading-snug line-clamp-2 mb-1">{event.title}</h4>
        {showVendor && event.vendor && (
          <p className="text-[11px] text-[#21a17a] font-medium mb-1.5 truncate">{event.vendor.businessName}</p>
        )}
